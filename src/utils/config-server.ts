...
export async function startConfServer(): Promise<string> {
      ...
      if (!confServer) {
         portfinder
            ...
            .then(port => {
               ...
               server.listen(port, () => {
                  console.log(
                     `Conf Server listening on http://localhost:${port}`,
                  );
...
function setInitConf(): BaseConf | undefined {
   if (config) {
      return [
         config.LANGUAGE,
         config.AUTO_START,
...
function setFinalConf(base: BaseConf): void {
   const lang = () => {
      if (base[0]) {
         return 'pt';
      } else {
   ...
   const oaiModel:
      ...
      | 'o1-preview'
      | 'o1-mini';
   const sttEngine: 'Picovoice' | 'Whisper' = (base[6] ?? 'Picovoice') as
      | 'Picovoice'
      | 'Whisper';
   const ttsEngine: 'Picovoice' | 'OpenAI' | 'Google' = (base[7] ??
