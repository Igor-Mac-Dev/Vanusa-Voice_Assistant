...
export default function rhinoHandler(
   cmd: [{ intent: string; [slot: string]: string }, boolean],
): void {
   try {
      const compCmd: boolean = cmd[1];
   } catch (err) {
...
async (
   cmd: Array<[{ intent: string; [slot: string]: string }, boolean]>,
): Promise<Array<any>> => {
   try {
      let compCmd = false;
      ...
      if (Array.isArray(record)) {
         ...
         compCmd = record[1];
         if (compCmd) {
            const compositeStt: string = await handlers.compositeCmd();
            if (compositeStt === 'cancel') {
               return 'cancel';
            } else {
               return [cmd, compositeStt];
            }
         } else {
            if (cmd) {
               return [cmd];
         ...
         const transcirpt: string = await handlers.stt(record);
         return transcirpt;
      }
   } catch (error) {
