...
export default class OAIcompletion extends OpenAICompletion {
   ...
   public async completion(input: string): Promise<string | null> {
      try {
         const response = await this.openai?.chat.completions.create({
            messages: [
               JSON.parse(
                  `{"role": "system", "content": "${this.config.OAI_ASSIST_DEFINITION} ${this.config.OAI_ASSIST_USER_DEFINITION}"}`,
               ),
         ...
         throw new CustomError(
            erro ? erro : "°OpenAi's API Completion failed: ",
            error,
         );
      }
   }

   public async compositeCompletion(
      input: string,
      intent: string,
      ...
      try {
         ...
         const messageContent: { [key: string]: any } = JSON.parse(
            message.content,
         );
