...
export default class rhinoHandler {
    ...
    async jsonizeTranscript(cmd) {
        try {
            ...
            throw new CustomError('*Rhino Handler jsonizeTranscript failed: ', error);
        }
    }
    async RhinoHandler(cmd) {
        try {
            const finalCmd = cmd;
...
class rhinoInfo {
    constructor() {
        this.processFilePath = path.resolve('./dist/process-files/');
        this.documentsPath = path.resolve(path.join(homedir(), 'Documents'), 'Vanusa');
    }
    getDocumentsPath() {
