...
const goInputHandle = async (input) => {
    if (Array.isArray(input)) {
        ...
        container.player.once('Audio_Queue_End', async () => {
            ...
            let tts;
            if (sttInput)
                asnwer = await goComplet(sttInput);
            else
    ...
    try {
        ...
        if (input[1]) {
            ...
            container.player.once('Audio_Queue_End', async () => {
                ...
                if (typeof compositeRecord !== 'string' &&
                    typeof cmd !== 'string' &&
                    compositeRecord.message === 'composite') {
                    cmd = await container.rhino.parseTranscript(cmd, compositeRecord);
